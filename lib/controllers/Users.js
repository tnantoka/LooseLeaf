...
module.exports = function(models) {
  ...
  this.loadUserWithPassword = function(req, res, next, id) {
    //var flag = req.session.user ? null : false;
    //models.User.findByIdWithPassword(id, flag, function(err, user) {
    models.User.findByIdWithPassword(id, function(err, user) {
      if (!user) return res.send(404);
      req.user = user;
      next();
    });
