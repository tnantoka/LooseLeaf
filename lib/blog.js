...
//        url: entry.trackbacks[j].url,
  ...
  for (var i = 0; i < entries.length; i++) {
    categories[entry.category].entries.push(entries[i].id);  
...
*/
  ...
  blog.entry.get = function(id, fn) {
    ...
    if (!valid) {
      return fn(new util.NotFound(id));
    }
/* deferred   
    util.readFile(path.entry + valid.id + '.json')
    .next(function(data) {
    ...
    .error(function(err) {
      fn(new util.InternalServerError(err.message));
    });
*/
  ...
  blog.entry.comment = function(id, comment, fn) {
    ...
    for(var i in comment) {
      if(!comment[i]) {
        return fn(new util.BadRequest(id));
      }
    }
    fs.readFile(path.entry + valid.id + '.json', 'UTF-8', function(err, data) {
      if (err) {
  ...
  blog.entry.list = function(offset, fn) {
    ...
    if (!valid) {
      return fn(new util.NotFound(offset));
    }
    var list = [];
  ...
  blog.tag.list = function(tag, fn) {
    if (!tags[tag]) {
      return fn(new util.NotFound(tag));
    }
    var list = [];
  ...
  blog.archive.monthly = function(year, month, fn) {
    ...
    if (!valid) {
      return fn(new util.NotFound(year + '-' + month));
    }
    var list = [];
  ...
  blog.archive.daily = function(year, month, date, fn) {
    ...
    if (!valid) {
      return fn(new util.NotFound(year + '-' + month + '-' + date));
    }
    var list = [];
  ...
  blog.entry.add = function(entry, fn) {
    for(var i in entry) {
      if(!entry[i] && i != 'tb' && i != 'tags' && i != 'private') {
        return fn(new util.BadRequest(i));
      }
    }
    console.dir(entry);
    var id = nextEntryId();
  ...
  blog.entry.edit = function(id, edit, fn) {
    for(var i in edit) {
      if(!edit[i] && i != 'tb' && i != 'tags' && i != 'private') {
        return fn(new util.BadRequest(i));
      }
    }
    console.dir(edit);
