...
//TODO: mappingは別ファイルにする
  ...
  for (i = 0; i < jsonFiles.length; i++) {
    ...
    for (var j = 0; j < entry.comments.length; j++) {
      comments.push({
        ...
        date: common.toReadableDate(entry.comments[j].date),
        id: entry.id
      });
    }
    
    categories[entry.category].entries.push(entry.id);
  ...
  function checkMonth(year, month) {
    if (isNaN(year) || isNaN(month) || month < 1 || month > 12 || !calendar[year] || !calendar[year][month]) {
      return;
    }
    return {
      year: parseInt(year),
      month: common.fillZero(parseInt(month), 2)
    };
  }
  ...
  function addCalendar(entry) {
    ...
    var m = common.fillZero(date.getMonth() + 1, 2);
    var d = common.fillZero(date.getDate(), 2);
  ...
  blog.archive.monthly = function(year, month, fn) {
    ...
    if (!valid) {
      return fn(new common.NotFound(year + '-' + month));
    }
    var list = [];
